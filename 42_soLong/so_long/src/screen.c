...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/19 22:59:35 by bkaramol          #+#    #+#             */
/*   Updated: 2022/11/19 23:28:56 by bkaramol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_make_map(t_data *data, char *map)
{
	...
	if (fd < 0)
		ft_error("File could not have opened", data);
...
void	ft_init_image(t_data *data)
{
	data->player_f = mlx_xpm_file_to_image(data->mlx, PL_FT, &data->imgx,
			&data->imgy);
	data->player_b = mlx_xpm_file_to_image(data->mlx, PL_BK, &data->imgx,
			&data->imgy);
	data->player_l = mlx_xpm_file_to_image(data->mlx, PL_LT, &data->imgx,
			&data->imgy);
	data->player_r = mlx_xpm_file_to_image(data->mlx, PL_RT, &data->imgx,
			&data->imgy);
