...
void *loops_for_philos(void *argument)
{
	...
	if (philo->philo_num == 1)
	{
		...
		/* bir yandan da sürekli filozof ölmüşse diye kontrol ediyoruz. bu
		fonksiyon eğer ölmüşse 1 değilse döndürür. ft_philo_check 1 döndürmediği
		sürece çalışır. */
	...
	while (1)
	{
		/* eğer ölmüşse break ile bu 3 eylemin art arda sürdürme sürecinden
		break ile çıkıyoruz */
...
void eat_time_philos(t_philo *philo)
{
	...
	/* opsiyonel yemek yeme sayısını 1 arttırıyoruz */
	philo->eaten++;
	pthread_mutex_unlock(philo->lock);
	/* ve diğer filoları bekleyen fonksiyon */
...
void sleep_time_philos(t_philo *philo)
{
	...
	diğerlerini bekliyor. bu fonksiyon bitene kadar sleep_time_philos()
